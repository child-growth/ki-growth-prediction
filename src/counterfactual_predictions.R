...
for(i in cohorts){
  if(!is.null(hal_fit_list[[i]]$Y)){
    res_ic_beta=NULL
    print(i)
    try(res_ic_beta <- get_ic_beta_cont(fit=hal_fit_list[[i]]$fit_init, X=hal_fit_list[[i]]$X, Y=hal_fit_list[[i]]$Y, n=hal_fit_list[[i]]$n))
    beta_ic_list[[i]] <- NA
    beta_ic_list[[i]] <- res_ic_beta
  }
}
length(beta_ic_list)
predicted_means$study[!(predicted_means$study %in% names(beta_ic_list))]
...
saveRDS(list(p=p1, plotdf=plotdf), paste0(here::here(),"/figures/waz_shift_example.RDS"))
